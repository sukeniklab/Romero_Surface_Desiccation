...
ax.hist(data['S/T_mean'],bins=bins,histtype='step',lw=3,color='k')
for quant in quants.values[1:-1]:
    ax.vlines(quant,0,1000,color='lightgrey',ls='--',zorder=0,lw=2)
ax.vlines(data['S/T_mean'].median(),0,1000,ls=':',color='k',zorder=3,lw=3)
ax.set_xlim(0,1)
ax.set_ylabel('protein count')
ax.set_xlabel('resolubility')
ax.set_ylim(0,400)
ax.text(0.5,350,'median = %.2f' % data['S/T_mean'].median(),fontsize=24)
